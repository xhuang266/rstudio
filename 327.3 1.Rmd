...
```{r}
#######
plot(area$land, area$farm, ylab = "Farm", xlab = "Land")
line = lm(area$farm ~ area$land)
abline(line, col = "limegreen")
########
k = 19000
x = area$land
y = area$farm
...
rho <- function(t) {
  r <- ifelse(abs(t) <= k, t ^ 2, 2 * k * abs(t) - k ^ 2)
  return(r)
}

Tukey = function(par, x, y){
  b0 = par[1]
  b1 = par[2]
  t = y - b0 - b1 * x
  rho = rho(t)
...
solve = optim (par = c(0,0), fn = Tukey, x = area$land, y = area$farm, method = "Nelder-Mead")
abline(a = solve$par[1], b = solve$par[2], col = "navy")

#######

Tukey.gradient = function(b, x, y) {
  b0 = b[1]
  b1 = b[2]
  t = y - b0 - b1 * x
  s.gradient = ifelse(abs(t) <= k, 2 * t, r <- 2 * k * sign(t))
  ds.db0 = -(sum(s.gradient)) / n
  ds.db1 = -(sum(s.gradient * x)) / n
  return(c(ds.db0, ds.db1))
}

m = optim(par = c(0, 0), fn = Tukey, x, y, gr = Tukey.gradient, method = "BFGS")
abline(a = m$par[1], b = m$par[2], col = "black")
n = optim(par = c(0, 0), fn = Tukey, x, y, gr = Tukey.gradient, method = "CG")
abline(a = n$par[1], b = n$par[2], col = "coral", lty = "dashed")
legend("topleft", legend = c("Least squares", "Nelder-Mead", "BFGS", "CG"), lty = c(1, 1, 1, 2), col = c("limegreen", "navy", "black", "coral"))


```

f. For which method was the value of the Tukey function the smallest?
**The BFGS method has the smallest value of the Tukey function.**
...
```{r}
curve(rho(x), -100000, 100000, n = 200001, xlab = "t", ylab = expression(rho(t)))
```
...
rm(list = ls())
yhat = function(b) {
  y.hat = numeric(length(nhtemp))
  y.hat[1] = nhtemp[1]
  for(i in 2:length(nhtemp)) {
    y.hat[i] <- b * nhtemp[i-1] + (1-b) * y.hat[i-1]
  }
  return(sum((nhtemp - y.hat)^2) / length(nhtemp))
}
opt = optimize(yhat, c(0, 1))
b = opt$minimum
b
```
...
plot(y = nhtemp, x = 1912:1971, ylab = "Temperature", xlab = "Year", "o")
lines(y = y.hat, x = 1912:1971, col = "red")
```
...
```{r}
y.hat1 = numeric(length(nhtemp))
y.hat2 = numeric(length(nhtemp))
y.hat1[1] = nhtemp[1]
y.hat2[1] = nhtemp[1]

for(i in 2: length(nhtemp)) {
  y.hat1[i] = 0.1 * nhtemp[i-1] + (1-0.1) * y.hat1[i-1]
  y.hat2[i] = 0.9 * nhtemp[i-1] + (1-0.9) * y.hat2[i-1]
}
plot(y = nhtemp, x = 1912:1971, ylab = "Temperature", xlab = "Year", "o")
lines(y = y.hat, x = 1912:1971, col = "red")
lines(y = y.hat1, x = 1912:1971, col = "purple")
lines(y = y.hat2, x = 1912:1971, col = "blue")
legend("bottomright", legend = c("original", "beta = 0.1860813", "beta = 0.1", "beta = 0.9"), col = c("black", "red", "purple", "blue"), lty = c(1, 1, 1, 1))
```
...
likelihood <- function(par) {
  mu <- par[1]
  sigma <- par[2]
  lgll <- n * log(1 / (sqrt(2 * pi) * sigma)) - sum((mtcars$mpg - mu)^2) / (2 * sigma ^ 2)
  return(lgll)
}
opt = optim(c(0, 1), fn = likelihood, control = list(fnscale = -1))
print(opt$par)
...
lglikelihood = function(par) {
  b0 = par[1]
  b1 = par[2]
  lgll = -sum(log(1 + exp(b0 + b1 * x))) + sum(y * (b0 + b1 * x))
  return(lgll)
}

opt = optim(c(0, 0), fn = lglikelihood, control = list(fnscale = -1))
b0 = opt$par[1]
b1 = opt$par[2]
print(opt$par)

plot(y = jitter(y), jitter(x), ylab = "reached.menarche", xlab = "age")
points(x = menarche$Age, y = menarche$Menarche / menarche$Total, col = "red")
curve(1 / (1 + exp(-(b0 + b1 * x))), min(menarche$Age), max(menarche$Age), add = TRUE, col = "purple")
